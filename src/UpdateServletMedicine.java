...
public class UpdateServletMedicine extends HttpServlet 
{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException 
	{
		...
		try 
		{
			...
			PreparedStatement ps=con.prepareStatement("update medicine_details set MEDICINE_NAME=?,MEDICINE_PRICE=?,MEDICINE_EXPIRY_DATE=?,STORE_ID=?,MEDICINE_TYPE_ID=? where ID=?");
			ps.setString(1,MEDICINE_NAME);
			ps.setFloat(2,MEDICINE_PRICE);
			ps.setString(3,MEDICINE_EXPIRY_DATE);
			ps.setInt(4,STORE_ID);
			ps.setInt(5,MEDICINE_TYPE_ID);
			ps.setInt(6,ID);
			ps.execute();
			System.out.println(ID);
			System.out.println(MEDICINE_NAME);
			System.out.println(MEDICINE_EXPIRY_DATE);
